...
% Usage:  >> ControlSystemsAnalyzer
    ...
    properties (Access = public)
        ...
        % Step Response
        StepResponseTab     matlab.ui.container.Tab
        StepAxes            matlab.ui.control.UIAxes
        StepMetricsLabel    matlab.ui.control.Label
    end
    ...
    methods (Access = private)
        ...
        function renderBlockDiagram(app, gcn, gcd_v, gn, gd, hn, hd, fbNeg)
            ...
            text(ax,sx-0.07,midY+0.08,'+','Color',wireC,'FontSize',12,'FontName',fontN);
            ...
            rectangle(ax,'Position',[4.8 midY-0.55 1.6 1.1],...
                'FaceColor',boxC,'EdgeColor',boxEdC,'LineWidth',1.5,'Curvature',0.1);
            text(ax,5.60,midY+0.28,'Plant','Color',labC,'HorizontalAlignment','center',...
                'FontSize',12,'FontName',fontN);
            ...
            if showH
                ...
                text(ax,5.00,fbY+0.17,'Sensor','Color',labC,'HorizontalAlignment','center',...
                    'FontSize',12,'FontName',fontN);
                ...
                plot(ax,[1.70 7.50],[fbY fbY],'Color',fbC,'LineWidth',2);
                plot(ax,[1.70 1.70],[fbY midY-0.22],'Color',fbC,'LineWidth',2);
                text(ax,4.60,fbY+0.22,'Unity Feedback  H(s) = 1',...
                    'Color',fbC,'HorizontalAlignment','center','FontSize',12,'FontName',fontN);
            end
            ...
            text(ax,3.30,midY-0.38,gcStr,'Color',[0.53 0.6 0.72],...
                'HorizontalAlignment','center','FontSize',11,'FontName',fontN,...
                'Interpreter','none');
        ...
        function renderRouth(app, charPoly)
            ...
            colNames{1} = 'Row';
            for j = 1:cols, colNames{j+1} = ['Col. ' num2str(j)]; end
            ...
            if hasZero
                app.StabilityLabel.Text = ...
                    ['⚠  MARGINAL STABILITY / Poles on jω-axis' newline ...
                     '   Zero pivot in Routh array'];
            ...
            elseif changes == 0
                ...
                app.StabilityLabel.Text = sprintf( ...
                    '✗  UNSTABLE — %d sign change(s)  →  %d pole(s) in RHP', ...
                    changes, changes);
                app.StabilityLabel.FontColor = [1 0.27 0.27];
            end

            app.CharEqNoteLabel.Text = ...
                ['Characteristic equation:  ' app.poly2str_local(charPoly) '  =  0'];
        end
        ...
        function renderPoles(app, CL, OL)
            ...
            types = [ repmat({'CL Pole'},  numel(clP),1);
                      repmat({'CL Zero'},  numel(clZ),1);
                      repmat({'OL Pole'},  numel(olP),1);
                      repmat({'OL Zero'},  numel(olZ),1) ];

            data = cell(numel(all_v), 6);
            for i = 1:numel(all_v)
                re = real(all_v(i)); im = imag(all_v(i));
                if re > 1e-6,                        loc = 'RHP (Unstable)';
                elseif abs(re)<1e-5 && abs(im)>1e-6, loc = 'jω-axis';
                else,                                 loc = 'LHP (Stable)'; end
                data{i,1} = types{i};
            ...
            app.PolesTableUI.ColumnName = {'Type','Re','Im','|ρ|','θ (°)','Location'};
        ...
        function renderRootLocus(app, OL, kMax, kSteps)
            ...
            nPoles = length(olD) - 1;
            if nPoles <= 0
                text(ax,0,0,'No open-loop poles found.',...
            ...
            for li = 1:nPoles
                ...
                for si = 1:size(segs,2)
                    ...
                    plot(ax, re_v(s1:s2), im_v(s1:s2), ...
                        ...
                        'DisplayName', ifelse(si==1, ['Locus ' num2str(li)], '_'));
                end
            end
        ...
        function renderBode(app, OL, wMin, wMax)
            ...
            try
                ...
                app.MarginsLabel.Text = 'Could not compute stability margins.';
        ...
        function renderNichols(app, OL, wMin, wMax)
            ...
            plot(ax,-180,0,'x','Color',[1 0.27 0.27],'MarkerSize',14,...
                'LineWidth',3,'DisplayName','Critical (−180°, 0 dB)');
        ...
        function renderStepResponse(app, CL)
            % ── Step response data ──────────────────────────────────────────
            try
                [y, t] = step(CL);
                y = squeeze(y); t = t(:);
            catch
                app.StepMetricsLabel.Text = 'Cannot compute step response for this system.';
                app.StepMetricsLabel.FontColor = [1 0.27 0.27];
                return;
            end

            % ── DC gain (steady-state value) ────────────────────────────────
            try
                yss = dcgain(CL);
                if ~isfinite(yss) || abs(yss) > 1e6, yss = y(end); end
            catch
                yss = y(end);
            end

            % ── stepinfo for time-domain specs ──────────────────────────────
            tr=NaN; tp=NaN; ts2=NaN; ts5=NaN; Mp=NaN;
            try
                si  = stepinfo(CL,'SettlingTimeThreshold',0.02);
                si5 = stepinfo(CL,'SettlingTimeThreshold',0.05);
                tr  = si.RiseTime;
                tp  = si.PeakTime;
                ts2 = si.SettlingTime;
                ts5 = si5.SettlingTime;
                Mp  = si.Overshoot;
            catch; end

            % ── ζ, ωn from dominant closed-loop poles ───────────────────────
            zeta = NaN; wn = NaN;
            try
                clPoles   = pole(CL);
                compMask  = abs(imag(clPoles)) > 1e-6;
                compPoles = clPoles(compMask);
                if ~isempty(compPoles)
                    % Dominant = smallest |Re(p)| (slowest transient)
                    [~, idx] = min(abs(real(compPoles)));
                    dp   = compPoles(idx);
                    wn   = abs(dp);
                    zeta = -real(dp) / wn;
                else
                    rp = real(clPoles(~compMask));
                    rp = rp(rp < 0);
                    if ~isempty(rp)
                        [~,idx] = min(abs(rp));
                        wn   = abs(rp(idx));
            ...
            % Step response curve
            plot(ax, t, y, 'Color', [0 0.78 0.88], 'LineWidth', 2.5, ...
                'DisplayName', 'y(t)');

            % Steady-state line + 2% band
            if isfinite(yss) && abs(yss) > 1e-9
                plot(ax, [t(1) t(end)], [yss yss], '--', ...
                    'Color', [0.94 0.65 0], 'LineWidth', 1.4, ...
                    'DisplayName', sprintf('y_{ss} = %.4g', yss));
                bC = [0 0.91 0.48];
                plot(ax, [t(1) t(end)], yss*[1.02 1.02], ':', ...
                    'Color', bC, 'LineWidth', 0.9, 'HandleVisibility','off');
                plot(ax, [t(1) t(end)], yss*[0.98 0.98], ':', ...
                    'Color', bC, 'LineWidth', 0.9, 'HandleVisibility','off');
            end

            yMax = max(y); yMin = min(y); ySpan = max(yMax - yMin, 1e-9);

            % Peak (th) marker
            if ~isnan(tp) && isfinite(tp) && tp > 0
                yp = max(y);
                plot(ax, tp, yp, 'v', 'Color', [1 0.40 0.53], ...
                    'MarkerSize', 10, 'MarkerFaceColor', [1 0.40 0.53], ...
                    'LineWidth', 1.5, 'DisplayName', ...
                    sprintf('t_h = %.4g s  (h%%=%.2g%%)', tp, Mp));
            end

            % Rise time vertical marker
            if ~isnan(tr) && isfinite(tr) && tr > 0
                plot(ax, [tr tr], [yMin-0.05*ySpan, yMax+0.05*ySpan], ':', ...
                    'Color', [0.71 0.56 1], 'LineWidth', 1.6, ...
                    'DisplayName', sprintf('t_r = %.4g s', tr));
            end

            % Settling time (2%) vertical marker
            if ~isnan(ts2) && isfinite(ts2) && ts2 > 0
                plot(ax, [ts2 ts2], [yMin-0.05*ySpan, yMax+0.05*ySpan], '--', ...
                    'Color', [0 0.91 0.48], 'LineWidth', 1.4, ...
                    'DisplayName', sprintf('t_s(2%%) = %.4g s', ts2));
            end

            xlabel(ax,'Time (s)','Color',[0.78 0.85 0.94],'FontSize',14);
            ylabel(ax,'Amplitude','Color',[0.78 0.85 0.94],'FontSize',14);
            title(ax,'Step Response — Closed-Loop T(s)', ...
                'Color',[0.78 0.85 0.94],'FontSize',13,'FontName','Courier New');
            leg = legend(ax,'show');
            leg.TextColor = [0.78 0.85 0.94];
            leg.Color     = [0.05 0.09 0.13];
            ...
            % ── Metrics label ────────────────────────────────────────────────
            fv = @(x) localFmt(x);
            function s = localFmt(x)
                if isnan(x) || ~isfinite(x), s = 'N/A';
            ...
            dampClass = '';
            if ~isnan(zeta)
                if     zeta < 0,    dampClass = ' (Unstable)';
                elseif zeta < 1,    dampClass = ' (Underdamped)';
                elseif zeta == 1,   dampClass = ' (Critically Damped)';
                else,               dampClass = ' (Overdamped)';
                end
            end

            line1 = sprintf('ζ = %s%s     ωn = %s rad/s     h%% = %s %%', ...
                fv(zeta), dampClass, fv(wn), fv(Mp));
            line2 = sprintf('tr = %s s     th = %s s     ts(2%%) = %s s     ts(5%%) = %s s', ...
                fv(tr), fv(tp), fv(ts2), fv(ts5));
            app.StepMetricsLabel.Text = [line1 newline line2];

            if ~isnan(Mp) && isfinite(Mp)
                if     Mp < 5,  app.StepMetricsLabel.FontColor = [0 0.91 0.48];
                elseif Mp < 20, app.StepMetricsLabel.FontColor = [0.94 0.65 0];
                else,           app.StepMetricsLabel.FontColor = [1 0.27 0.27];
                end
            else
                app.StepMetricsLabel.FontColor = [0.78 0.85 0.94];
            end
        end
        ...
        function panel = makeTFPanel(app, parent, row, titleStr)
            ...
            panel.FontSize  = 13; panel.FontWeight = 'bold';
            panel.Layout.Row = row; panel.Layout.Column = 1;
        end

        function [numEF, denEF] = addTFFields(app, panel, defNum, defDen)
            g = uigridlayout(panel);
            g.ColumnWidth = {100,'1x'}; g.RowHeight = {30,30};
            g.BackgroundColor = [0.05 0.09 0.13];
            g.Padding = [6 6 6 6]; g.RowSpacing = 4; g.ColumnSpacing = 6;

            nLbl = uilabel(g,'Text','Numerator:');
            nLbl.FontColor = [0.53 0.60 0.72]; nLbl.FontSize = 12;
            ...
            numEF.FontName = 'Courier New'; numEF.FontSize = 14;
            numEF.Layout.Row = 1; numEF.Layout.Column = 2;
            numEF.ValueChangedFcn = createCallbackFcn(app, @CalcButtonPushed, true);

            dLbl = uilabel(g,'Text','Denominator:');
            dLbl.FontColor = [0.53 0.60 0.72]; dLbl.FontSize = 12;
            ...
            denEF.FontName = 'Courier New'; denEF.FontSize = 14;
            denEF.Layout.Row = 2; denEF.Layout.Column = 2;
            denEF.ValueChangedFcn = createCallbackFcn(app, @CalcButtonPushed, true);
        end
        ...
        function createComponents(app)
            ...
            app.SettingsGroupBox.FontSize        = 13;
            ...
            app.FeedbackButtonGroup.SelectionChangedFcn = ...
                createCallbackFcn(app, @CalcButtonPushed, true);

            app.NegFeedbackButton = uiradiobutton(app.FeedbackButtonGroup,...
                'Text','Negative Feedback (−)','Value',true,...
                'FontColor',[0.78 0.85 0.94],'FontSize',13);
            app.NegFeedbackButton.Position = [4 5 200 22];

            app.PosFeedbackButton = uiradiobutton(app.FeedbackButtonGroup,...
                'Text','Positive Feedback (+)','Value',false,...
                'FontColor',[0.78 0.85 0.94],'FontSize',13);
            app.PosFeedbackButton.Position = [210 5 180 22];

            % Row 2: K max, K steps
            l1=uilabel(sg,'Text','K max (R.L.):');
            l1.FontColor=[0.53 0.60 0.72];l1.FontSize=12;
            ...
            app.KMaxEditField.FontName='Courier New';app.KMaxEditField.FontSize=13;
            app.KMaxEditField.Layout.Row=2;app.KMaxEditField.Layout.Column=2;
            app.KMaxEditField.ValueChangedFcn = createCallbackFcn(app,@CalcButtonPushed,true);

            l2=uilabel(sg,'Text','K steps:');
            l2.FontColor=[0.53 0.60 0.72];l2.FontSize=12;
            ...
            app.KStepsEditField.FontName='Courier New';app.KStepsEditField.FontSize=13;
            app.KStepsEditField.Layout.Row=2;app.KStepsEditField.Layout.Column=4;
            app.KStepsEditField.ValueChangedFcn = createCallbackFcn(app,@CalcButtonPushed,true);

            % Row 3: w min, w max
            l3=uilabel(sg,'Text','Bode ω (10^n) from:');
            l3.FontColor=[0.53 0.60 0.72];l3.FontSize=12;
            ...
            app.WMinEditField.FontName='Courier New';app.WMinEditField.FontSize=13;
            app.WMinEditField.Layout.Row=3;app.WMinEditField.Layout.Column=2;
            app.WMinEditField.ValueChangedFcn = createCallbackFcn(app,@CalcButtonPushed,true);

            l4=uilabel(sg,'Text','to (10^n):');
            l4.FontColor=[0.53 0.60 0.72];l4.FontSize=12;
            ...
            app.WMaxEditField.FontName='Courier New';app.WMaxEditField.FontSize=13;
            app.WMaxEditField.Layout.Row=3;app.WMaxEditField.Layout.Column=4;
            app.WMaxEditField.ValueChangedFcn = createCallbackFcn(app,@CalcButtonPushed,true);

            % Row 4: note
            note=uilabel(sg,'Text',...
                'Enter coefficients from highest to lowest power, separated by spaces. e.g. "1 3 2" → s²+3s+2');
            note.FontColor=[0.35 0.45 0.60];note.FontSize=11;
            note.WordWrap='on';
            ...
            %% ─ Tab: TF Results ─
            app.TFResultsTab = uitab(app.ResultsTabGroup,'Title','T.F. Results');
            ...
            function lbl = makeLabel(parent,row,txt)
                ...
                lbl.FontColor=[0.94 0.65 0];lbl.FontSize=14;lbl.FontWeight='bold';
                lbl.Layout.Row=row;lbl.Layout.Column=1;
            end
            function ta = makeTA(parent,row,defVal)
                ta = uitextarea(parent,'Value',{defVal});
                ta.FontName='Courier New';ta.FontSize=15;
                ta.FontColor=[0 0.91 0.48];
                ta.BackgroundColor=[0.02 0.05 0.10];
                ta.Layout.Row=row;ta.Layout.Column=1;
            end
            makeLabel(tfg,1,'Open Loop — Gc(s)·G(s)·H(s)');
            app.TFOpenLoopText   = makeTA(tfg,2,'Waiting for calculation...');
            makeLabel(tfg,3,'Closed Loop — T(s)');
            app.TFClosedLoopText = makeTA(tfg,4,'Waiting for calculation...');
            app.TFClosedLoopText.FontColor = [0 0.78 0.88];
            makeLabel(tfg,5,'Characteristic Equation');
            app.TFCharEqText = makeTA(tfg,6,'Waiting for calculation...');
            ...
            app.RouthTableUI.Data = {'Waiting...'};
            app.RouthTableUI.BackgroundColor = [0.05 0.09 0.13; 0.07 0.12 0.20];
            app.RouthTableUI.ForegroundColor = [0 0.78 0.88];
            app.RouthTableUI.FontName = 'Courier New'; app.RouthTableUI.FontSize = 14;
            app.RouthTableUI.Layout.Row=1; app.RouthTableUI.Layout.Column=1;

            app.StabilityLabel = uilabel(rg,'Text','Waiting for calculation...');
            app.StabilityLabel.FontColor  = [0.53 0.60 0.72];
            app.StabilityLabel.FontSize   = 16; app.StabilityLabel.FontWeight='bold';
            ...
            app.MarginsLabel = uilabel(bodeg,'Text','Waiting for calculation...');
            app.MarginsLabel.FontColor=[0.78 0.85 0.94];app.MarginsLabel.FontSize=14;
            ...
            %% ─ Tab: Step Response ─
            app.StepResponseTab = uitab(app.ResultsTabGroup,'Title','Step Response');
            app.StepResponseTab.BackgroundColor = BG;
            stepGrid = uigridlayout(app.StepResponseTab);
            stepGrid.ColumnWidth = {'1x'}; stepGrid.RowHeight = {60,'1x'};
            stepGrid.BackgroundColor = BG; stepGrid.Padding = [10 8 10 8];
            stepGrid.RowSpacing = 4;

            app.StepMetricsLabel = uilabel(stepGrid,'Text','Waiting for calculation...');
            app.StepMetricsLabel.FontColor  = [0.78 0.85 0.94];
            app.StepMetricsLabel.FontSize   = 14;
            app.StepMetricsLabel.FontName   = 'Courier New';
            app.StepMetricsLabel.WordWrap   = 'on';
            app.StepMetricsLabel.Layout.Row = 1;
            app.StepMetricsLabel.Layout.Column = 1;

            stepAxGrid = uigridlayout(stepGrid);
            stepAxGrid.ColumnWidth = {'1x'}; stepAxGrid.RowHeight = {'1x'};
            stepAxGrid.Padding = [0 0 0 0]; stepAxGrid.BackgroundColor = BG;
            stepAxGrid.Layout.Row = 2; stepAxGrid.Layout.Column = 1;

            app.StepAxes = uiaxes(stepAxGrid);
            app.StepAxes.Layout.Row = 1; app.StepAxes.Layout.Column = 1;
            app.StepAxes.Color    = [0.027 0.063 0.118];
            app.StepAxes.XColor   = [1 1 1];
            app.StepAxes.YColor   = [1 1 1];
            app.StepAxes.GridColor= [0.12 0.20 0.35];
            app.StepAxes.FontName = 'Courier New';
    ...
    methods (Access = private)
        ...
        function CalcButtonPushed(app, ~)
            ...
            try
                ...
                app.renderStepResponse(CL);

            catch err
                uialert(app.UIFigure, ...
                    ['Error: ' err.message newline ...
                     'Check the input coefficients.'], ...
                    'Calculation Error', 'Icon','error');
            end
